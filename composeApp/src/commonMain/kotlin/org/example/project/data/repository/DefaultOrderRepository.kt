...
class DefaultOrderRepository(
    ...
    override suspend fun placeOrder(order: Order) = storefrontDao.placeOrder(order.toLocalEntity())

    override fun observeOrders() = storefrontDao.observeOrders()
        .map { orders -> orders.map { it.toDomainModel() } }

    override suspend fun getOrder(id: String) = storefrontDao.getOrder(id)?.toDomainModel()
}
